...
| analytic    | Set to true to switch the match stats report to a more detailed version like the Analytic Match Reports (see below|). Note that the _players_ status components are automatically hidden when this setting is enabled. The data in the report is refreshed on page load and then every 5 seconds. |
